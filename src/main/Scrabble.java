...
public final class Scrabble extends java.applet.Applet implements ItemListener {
    ...
    private final JImage boardImageUI = new JImage(GlobalCache.ImageCache.BOARD);
    private final JImage playRackImageUI = new JImage(GlobalCache.ImageCache.RACK);
    private final JImage throwRackImageUI = new JTextImage("exchange  *  exchange  *  exchange", GlobalCache.ImageCache.RACK, 0.5f, 0.55f);
    
    public final JPieceHolder jPieceHolder = new JPieceHolder();
    private final CLabel bagLabel = CLabel.newInstance();
    private final CLabel status = CLabel.newInstance();
    ...
    public class ClockTimer extends Timer implements ActionListener {
        ...
        public void start() {
            ...
            if(currentPlayer != null && currentPlayer.gameTime >= 0)
                ...
                gameTime = Math.max(timeForGame, 0);
            moveTime = Math.max(timeForMove, 0);
            super.start();
        }
